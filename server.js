...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    res.json({ ok: true, token, user: { role: "EMPLOYEE", employeeId: employee.id, employeeCode: employee.employee_code, username: employee.username, name: employee.name } });
  } catch (err) {
...
app.get("/api/auth/me", requireAuth, async (req, res) => {
  try {
    ...
    if (req.auth.role === "EMPLOYEE") {
      ...
      const { data } = await supabase.from("employee_permissions").select("permission").eq("employee_id", req.auth.employeeId);
...
async function decrementStockAtomic(productId, quantity) {
  ...
  const { data, error } = await supabase.from("products").update({ stock: newStock }).eq("id", productId).gte("stock", quantity).select("*");
  if (error) throw error;
  if (!data || data.length === 0) throw new Error("Concurrent stock conflict. Please try again.");
  return data[0];
}
...
async function changeOrderStatus(orderId, newStatus, actorName) {
  ...
  if (newStatus === "CONFIRMED") {
    ...
    if (productId) await decrementStockAtomic(productId, quantity);
...
app.post("/api/advertisements/:id/publish", requirePermission("advertising"), async (req, res) => {
  try {
    ...
    const replyMarkup = { inline_keyboard: [[{ text: ad.button_text || "ይህን ይዘዙን", url: buttonUrl }]] };
...
async function findPendingOrder(chatId) {
  if (!supabase) return null;
  const { data } = await supabase.from("orders").select("*").eq("telegram_chat_id", String(chatId)).in("status", ["NEW", "PAYMENT_PENDING", "RECEIPT_PENDING", "CONFIRMED", "DELIVERY_PENDING"]).order("created_at", { ascending: false }).limit(1);
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (data.startsWith("product_select_") || data.startsWith("product_")) {
        ...
        const product = await getProduct(productId);
        if (!product) { await sendMessage(chatId, "❌ እቃው አልተገኘም።"); return; }
      ...
      if (data === "order_confirm") {
        ...
        if (ADMIN_CHAT_ID) {
          await sendMessage(ADMIN_CHAT_ID, `🆕 <b>NEW ORDER</b>\n\n🆔 ${order.id}\n🛍 ${productName(p)}\n👤 ${session.name}\n📱 ${session.phone}\n🔢 ${quantity}\n💰 ${total} ETB`, {
            parse_mode: "HTML",
    ...
    if (textMessage === "/start" || textMessage.startsWith("/start ")) {
      ...
      if (startParam.startsWith("product_")) {
        ...
        const product = await getProduct(productId);
        if (!product || productStock(product) <= 0) { await sendMessage(chatId, "❌ እቃው አልተገኘም ወይም አልቋል።"); return; }
        userSessions[chatId] = { productId, product };
      ...
      if (startParam === "catalog") {
        ...
        await sendMessage(chatId, "🛒 የሚፈልጉትን ምርት ከታች ይምረጡ፦", { reply_markup: { inline_keyboard: keyboard } });
        return;
      }
      await sendMessage(chatId, "🛒 እንኳን ወደ <b>UNI MARKET</b> በደህና መጡ!", { parse_mode: "HTML" });
      return;
    }
    ...
    if (session) {
      if (session.step === "NAME") {
        ...
        session.step = "PHONE";
        await sendMessage(chatId, "📞 እባክዎ ስልክ ቁጥርዎን ይጻፉ።");
        return;
      }
      if (session.step === "PHONE") {
        session.phone = textMessage;
        session.step = "ADDRESS";
        await sendMessage(chatId, "📍 እባክዎ የመላኪያ አድራሻዎን ይጻፉ።");
        return;
      }
