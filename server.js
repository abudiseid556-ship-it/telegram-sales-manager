...
async function triggerAutoAd() {
  try {
    if (!supabase) return;
    // ስቶካቸው ከ 0 በላይ የሆኑ ምርቶችን እንመርጣለን
...
// በየ 6 ሰዓቱ እራሱ ማስታወቂያ እንዲለቅ (6 hours = 6 * 60 * 60 * 1000 ms)
...
app.post("/api/employees", requirePermission("employees"), async (req, res) => {
  try {
    ...
    // የሰራተኛውን ፈቃዶች (Permissions) ማስገባት
...
async function processTelegram(update) {
  try {
    ...
    if (textMessage === "🛒 ምርቶች") {
      const { data: prods } = await supabase.from("products").select("*").gt("stock", 0).limit(10);
      if (!prods || prods.length === 0) { await sendMessage(chatId, "❌ ምንም የሚገኝ እቃ የለም (Stock is empty)።"); return; }
      const keyboard = prods.prods ? [] : prods.map(p => [{ text: `${productName(p)} - ${productSellPrice(p)} ETB`, callback_data: `product_select_${p.id}` }]);
      await sendMessage(chatId, "🛒 የሚፈልጉትን ምርት ከታች ይምረጡ፦", { reply_markup: { inline_keyboard: keyboard } });
      return;
    }
