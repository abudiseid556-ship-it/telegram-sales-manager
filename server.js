...
/* =========================================================
   ADVERTISEMENTS API (WITH CATALOG & SPECIFIC PRODUCT LINKS)
========================================================= */
...
app.post("/api/advertisements", requirePermission("advertising"), async (req, res) => {
  try {
    ...
    const row = {
      ...
      text: body.text || null,
      photo_url: firstDefined(body.photoUrl, body.photo_url, body.photo),
      button_text: body.buttonText || body.button_text || "ይህን ይዘዙን",
...
app.post("/api/advertisements/:id/publish", requirePermission("advertising"), async (req, res) => {
  try {
    ...
    const buttonUrl = productId 
      ? `https://t.me/${BOT_USERNAME}?start=product_${productId}` 
      : `https://t.me/${BOT_USERNAME}?start=catalog`;

    const replyMarkup = {
      inline_keyboard: [[{ text: ad.button_text || "ይህን ይዘዙን", url: buttonUrl }]]
    };
    ...
    if (ad.photo_url) {
      ...
      result = await sendMessage(chatId, ad.text || ad.title, { reply_markup });
    }
...
app.get("/api/reports", requirePermission("reports"), async (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      summary: { totalOrders: orders.length, deliveredOrders, pendingOrders, totalSales, totalProfit },
      orders
    });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
...
/* =========================================================
   TELEGRAM WEBHOOK PROCESSING (WITH CATALOG & DEEP LINKING)
========================================================= */
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (data.startsWith("product_select_")) {
        const productId = data.replace("product_select_", "");
        const product = await getProduct(productId);
        if (!product) {
    ...
    if (textMessage === "/start" || textMessage.startsWith("/start ")) {
      ...
      if (startParam === "catalog") {
        ...
        if (!prods || prods.length === 0) {
          await sendMessage(chatId, "❌ ምንም የሚገኝ እቃ የለም።");
          return;
        }

        const keyboard = prods.map(p => [{ text: `${productName(p)} - ${productSellPrice(p)} ETB`, callback_data: `product_select_${p.id}` }]);
        await sendMessage(chatId, "🛒 የሚፈልጉትን ምርት ከታች ይምረጡ፦", {
          reply_markup: { inline_keyboard: keyboard }
        });
        return;
      }

      await sendMessage(chatId, "🛒 እንኳን ወደ <b>UNI MARKET</b> በደህና መጡ! ከታች ያለውን ሊንክ በመጫን ምርቶችን ማየት ይችላሉ።", { parse_mode: "HTML" });
      return;
    }
