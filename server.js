...
async function getTelegramSettings(){
 ...
 return data||{
  id:null,
  ad_chat_id:"",
  bot_username:(process.env.TELEGRAM_BOT_USERNAME||"uni_market_shop_bot").replace(/^@/,"")
 };
}
...
app.post("/api/telegram-settings",async(req,res)=>{
 try{
  const adChatId=String(req.body.ad_chat_id||"").trim();
...
app.post("/api/upload",(req,res,next)=>{
 ...
 try{
  if(!req.file){
   return res.status(400).json({error:"ፎቶ አልተገኘም።"});
  }
...
app.post("/api/products",async(req,res)=>{
 try{
  ...
  if(!name?.trim()){
   return res.status(400).json({error:"የምርት ስም ያስፈልጋል።"});
  }
...
app.post("/api/advertisements/:id/publish",async(req,res)=>{
 try{
  ...
  if(!chatId){
   return res.status(400).json({
    error:"Dashboard → Telegram Settings ላይ Channel/Group ID አስገባ።"
   });
  }
  ...
  if(product){
   ...
   caption+=`💰 ${Number(product.sell_price||0).toLocaleString()} ETB\n`;

   if(Number(ad.old_price)>0){
    caption+=`❌ ${Number(ad.old_price).toLocaleString()} ETB\n`;
   }

   if(Number(ad.discount)>0){
    caption+=`🔥 ${Number(ad.discount)}% OFF\n`;
   }
  ...
  const startParam=
   ad.product_id
   ?`?start=product_${encodeURIComponent(String(ad.product_id))}`
   :"";

  const keyboard={
   inline_keyboard:[[
    {
     text:ad.button_text||"🛒 በዚህ ይዘዙን",
     url:`https://t.me/${username}${startParam}`
    }
   ]]
  };
  ...
  if(ad.photo_url){
   sent=await bot.sendPhoto(chatId,ad.photo_url,{
    caption,
...
async function confirmOrder(order){
 ...
 if(product){
  ...
  if(stock<quantity){
   throw new Error(`Stock አይበቃም። ያለው Stock: ${stock}`);
  }
...
async function getPaymentMessage(total){
 ...
 if(payment.account_name){
  text+=`👤 ${payment.account_name}\n`;
 }

 if(payment.account_number){
  text+=`🔢 ${payment.account_number}\n`;
 }
   ...
   if(param.startsWith("product_")){
    ...
    if(product&&Number(product.stock||0)>0){
     ...
     return bot.sendMessage(
      chatId,
      `🛍️ ${product.name}\n\n`+
      `💰 ${Number(product.sell_price||0).toLocaleString()} ETB\n`+
      `📦 Stock: ${product.stock}\n\n`+
      `🔢 ስንት ይፈልጋሉ?`
     );
    }
 ...
 bot.on("message",async msg=>{
  try{
   ...
   if(session.step==="QUANTITY"){
    ...
    if(quantity>Number(session.product?.stock||0)){
     return bot.sendMessage(
      chatId,
      `❌ በቂ Stock የለም። ያለው: ${session.product?.stock||0}`
     );
    }
   ...
   if(session.step==="ADDRESS"){
    ...
    const total=
     Number(p.sell_price||0)*session.quantity;
 ...
 bot.on("callback_query",async query=>{
  try{
   ...
   if(data.startsWith("product_")){
    const id=data.slice("product_".length);
   ...
   if(data==="confirm_order"){
    ...
    const total=
     Number(p.sell_price||0)*current.quantity;

    const profit=
     (Number(p.sell_price||0)-Number(p.buy_price||0))*
     current.quantity;
    ...
    const payment=await getPaymentMessage(total);
