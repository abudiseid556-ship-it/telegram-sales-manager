...
if (SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
  ...
  console.error("WARNING: SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY is missing.");
}
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (username === MASTER_ADMIN_USERNAME && MASTER_ADMIN_PASSWORD && password === MASTER_ADMIN_PASSWORD) {
      ...
      return res.json({ ok: true, token, user: { role: "MASTER_ADMIN", username, name: "Master Admin" } });
    }
    ...
    if (!employee || employee.active === false) {
      return res.status(401).json({ ok: false, error: "Invalid credentials or inactive account" });
    }
...
// CLEANED AND FIXED /api/auth/me ROUTE
app.get("/api/auth/me", requireAuth, async (req, res) => {
  try {
    ...
    if (req.auth.role === "EMPLOYEE") {
      ...
      user.permissions = (data || []).map((x) => x.permission).filter(Boolean);
    } else {
      user.permissions = ["*"];
    }
    ...
    if (error) {
      if (/Invalid compact JWS/i.test(error.message)) {
        return res.status(500).json({
          ok: false,
          error: "Supabase Storage auth failed. Check SUPABASE_SERVICE_ROLE_KEY in Render."
        });
      }
...
/* =========================================================
   ORDERS API & ATOMIC STOCK DECREMENT
========================================================= */
...
async function decrementStockAtomic(productId, quantity) {
  const product = await getProduct(productId);
  if (!product) throw new Error("Product not found");

  const currentStock = productStock(product);
  if (currentStock < quantity) throw new Error("Not enough stock available");
  ...
  if (!data || data.length === 0) {
    throw new Error("Concurrent stock conflict. Please try again.");
  }
  return data[0];
}
...
async function changeOrderStatus(orderId, newStatus, actorName) {
  ...
  const oldStatus = safeString(order.status).toUpperCase();
  newStatus = safeString(newStatus).toUpperCase();

  if (newStatus === "CONFIRMED") {
    if (!["RECEIPT_PENDING", "PAYMENT_PENDING", "NEW"].includes(oldStatus)) {
    ...
    if (!productId) throw new Error("Order has no product ID");

    await decrementStockAtomic(productId, quantity);
    ...
    if (customerChatId) {
      await sendMessage(
        ...
        `✅ ክፍያዎ ተረጋግጧል!\n\n📦 እቃዎ በማድረስ ሂደት ላይ ነው።\n\nምርቱ ሲደርስዎት <b>ደርሶኛል</b> የሚለውን ቁልፍ ይጫኑ።`,
        {
          parse_mode: "HTML",
          reply_markup: {
            inline_keyboard: [[{ text: "📦 ደርሶኛል", callback_data: `order_received_${data.id}` }]]
          }
        }
      );
    }
  ...
  if (newStatus === "REJECTED") {
    const { data, error } = await supabase
      .from("orders")
      .update({ status: "REJECTED", payment_status: "REJECTED", rejected_at: nowISO() })
      .eq("id", orderId)
      .select("*")
      .single();
    ...
    const customerChatId = firstDefined(data.telegram_chat_id, data.customer_id);
    if (customerChatId) {
      await sendMessage(customerChatId, "❌ የክፍያ ደረሰኝዎ አልተረጋገጠም። እባክዎ ትክክለኛ ደረሰኝ ይላኩ።");
    }
    return data;
  }
  ...
  if (newStatus === "DELIVERED" || newStatus === "CLOSED") {
    const { data, error } = await supabase
      .from("orders")
      .update({ status: "DELIVERED", delivery_status: "DELIVERED", delivered_at: nowISO(), closed_at: nowISO() })
      .eq("id", orderId)
      .select("*")
      .single();
...
/* =========================================================
   RECEIPT PROXY API
========================================================= */
app.get("/api/orders/:id/receipt", requirePermission("view_orders"), async (req, res) => {
  try {
    const order = await getOrderById(req.params.id);
    if (!order || !order.receipt_file_id) return res.status(404).send("Receipt not found");

    const fileInfo = await telegram("getFile", { file_id: order.receipt_file_id });
    if (!fileInfo?.file_path) return res.status(404).send("Telegram file not found");

    const imgRes = await fetch(`https://api.telegram.org/file/bot${BOT_TOKEN}/${fileInfo.file_path}`);
    if (!imgRes.ok) return res.status(500).send("Could not download receipt");
...
app.post("/api/advertisements/:id/publish", requirePermission("advertising"), async (req, res) => {
  try {
    ...
    const buttonUrl = productId ? `https://t.me/${BOT_USERNAME}?start=product_${productId}` : null;
    const replyMarkup = buttonUrl ? { inline_keyboard: [[{ text: ad.button_text || "Order Now", url: buttonUrl }]] } : undefined;

    let result;
    if (ad.photo_url) {
      result = await telegram("sendPhoto", { chat_id: chatId, photo: ad.photo_url, caption: ad.text || ad.title, reply_markup: replyMarkup });
    } else {
      result = await sendMessage(chatId, ad.text || ad.title, reply_markup ? { reply_markup } : {});
    }
...
/* =========================================================
   REPORTS API
========================================================= */
app.get("/api/reports", requirePermission("reports"), async (req, res) => {
  try {
...
async function findPendingOrder(chatId) {
  ...
  const { data } = await supabase
    ...
    .in("status", ["NEW", "PAYMENT_PENDING", "RECEIPT_PENDING", "CONFIRMED", "DELIVERY_PENDING"])
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (data.startsWith("admin_confirm_")) {
        if (!ADMIN_CHAT_ID || String(fromId) !== String(ADMIN_CHAT_ID)) return;
        await changeOrderStatus(data.replace("admin_confirm_", ""), "CONFIRMED", "Telegram Admin");
        return;
      }

      if (data.startsWith("admin_reject_")) {
        if (!ADMIN_CHAT_ID || String(fromId) !== String(ADMIN_CHAT_ID)) return;
        await changeOrderStatus(data.replace("admin_reject_", ""), "REJECTED", "Telegram Admin");
        return;
      }
      ...
      if (data.startsWith("order_received_")) {
        ...
        const order = await getOrderById(orderId);
        if (!order || String(order.telegram_chat_id) !== String(chatId)) return;

        await changeOrderStatus(orderId, "DELIVERED", `Telegram User ${fromId}`);
        delete userSessions[chatId];
        await sendMessage(chatId, "🎉 እናመሰግናለን! ኦርደርዎ ተዘግቷል። ❤️");
        return;
      }
      ...
      if (data.startsWith("product_")) {
        ...
        await sendMessage(chatId, `🛍 <b>${productName(product)}</b>\n\n💰 ዋጋ: ${productSellPrice(product)} ETB\n\nየሚፈልጉትን ብዛት ይምረጡ፦`, {
          ...
          reply_markup: {
            inline_keyboard: [
              [{ text: "1", callback_data: "qty_1" }, { text: "2", callback_data: "qty_2" }, { text: "3", callback_data: "qty_3" }]
            ]
          }
        });
        return;
      ...
      if (data.startsWith("qty_")) {
        ...
        session.step = "NAME";
        await sendMessage(chatId, "👤 እባክዎ ሙሉ ስምዎን (Full Name) ይጻፉ።");
        return;
      }

      if (data === "order_confirm") {
        const session = userSessions[chatId];
        if (!session || !session.product) return;
        ...
        const paymentText = `🧾 ኦርደርዎ ተመዝግቧል!\n\n` +
          ...
          `💰 ጠቅላላ: ${total} ETB\n\n` +
          `🏦 ባንክ: ${payment.bank_name || "Commercial Bank of Ethiopia"}\n` +
          `👤 ስም: ${payment.account_name || "UNI MARKET"}\n` +
          `💳 አካውንት: ${payment.account_number || "1000..."}\n\n` +
          `ክፍያውን ከፈጸሙ በኋላ የክፍያ ደረሰኙን (Receipt) ፎቶ ይላኩ።`;

        await sendMessage(chatId, paymentText);

        if (ADMIN_CHAT_ID) {
          const adminText = `🆕 <b>NEW ORDER</b>\n\n🆔 ${order.id}\n🛍 ${productName(p)}\n👤 ${session.name}\n📱 ${session.phone}\n🔢 ${quantity}\n💰 ${total} ETB`;
          await sendMessage(ADMIN_CHAT_ID, adminText, {
            parse_mode: "HTML",
            reply_markup: {
              inline_keyboard: [[
                { text: "✅ Confirm", callback_data: `admin_confirm_${order.id}` },
                { text: "❌ Reject", callback_data: `admin_reject_${order.id}` }
              ]]
            }
          });
        }
    ...
    if (message.photo && message.photo.length) {
      ...
      if (!order) {
        await sendMessage(chatId, "❌ የሚጠብቅ ኦርደር የለም።");
        return;
      }
      ...
      if (ADMIN_CHAT_ID) {
        ...
        try {
          await telegram("sendPhoto", { chat_id: ADMIN_CHAT_ID, photo: photo.file_id, caption: `Receipt for Order ${order.id}` });
        } catch (e) {}
      }
      return;
    }
    ...
    if (textMessage === "/start" || textMessage.startsWith("/start ")) {
      ...
      if (startParam.startsWith("product_")) {
        ...
        if (!product || productStock(product) <= 0) {
          await sendMessage(chatId, "❌ እቃው አልተገኘም ወይም አልቋል።");
          return;
        }

        userSessions[chatId] = { productId, product };
        await sendMessage(chatId, `🛍 <b>${productName(product)}</b>\n\n💰 ዋጋ: ${productSellPrice(product)} ETB\n📦 የቀረ ክምችት: ${productStock(product)}`, {
          parse_mode: "HTML",
          reply_markup: {
            inline_keyboard: [
              [{ text: "1", callback_data: "qty_1" }, { text: "2", callback_data: "qty_2" }, { text: "3", callback_data: "qty_3" }]
      ...
      await sendMessage(chatId, "🛒 እንኳን ወደ <b>UNI MARKET</b> በደህና መጡ! የሚፈልጉትን ምርት ከታች ባለው ሊንክ ይመልከቱ።", { parse_mode: "HTML" });
      return;
    }
    ...
    if (session) {
      if (session.step === "NAME") {
        ...
        session.step = "PHONE";
        await sendMessage(chatId, "📞 እባክዎ ስልክ ቁጥርዎን (Phone Number) ይጻፉ።");
        return;
      }
      if (session.step === "PHONE") {
        session.phone = textMessage;
        session.step = "ADDRESS";
        await sendMessage(chatId, "📍 እባክዎ የመላኪያ አድራሻዎን (Delivery Address) ይጻፉ።");
        return;
      }
      if (session.step === "ADDRESS") {
        ...
        await sendMessage(chatId, `🧾 <b>የኦርደር ማረጋገጫ</b>\n\n🛍 እቃ: ${productName(p)}\n🔢 ብዛት: ${qty}\n👤 ስም: ${session.name}\n📞 ስልክ: ${session.phone}\n📍 አድራሻ: ${session.address}\n💰 ድምር: ${total} ETB`, {
          ...
          reply_markup: {
            inline_keyboard: [
              [{ text: "✅ ኦርደር አረጋግጥ", callback_data: "order_confirm" }],
